...
> 更新：2026-09-23；工作区：`E:\9.1agent_exam`；当前分支：`agent+api`；Agent 配置扩展、真实试跑、owner 登记与网页选择验收均已完成，相关提交已快进推送到 `origin/agent+api`。本地 `main` 由另一 worktree 使用；Git 现状以现场命令为准。
>
...
| 正式运行态 | 2026-09-23 12:35 +08:00 按用户要求停止旧进程并完整重启：专属 PostgreSQL/MinIO、当前 Web 3000、Backend 8000、唯一持续 Worker 与私有 HTTPS 均可用；本机 Web、OpenAPI、私有 HTTPS 为 200，匿名 Web→API 为预期 401，旧 PID 均不存在。主库保持 4 个 Job、8 个 Run、3 个启用 Agent，排队与活动 Job 均为 0 | 两种新配置的六题隔离原型试跑、owner 登记和真实网页选择均已完成；网页核对未提交 Job。动态进程状态使用前仍须现场复查 |
